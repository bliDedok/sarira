...
describe('Phase 6 Guided Meal and Flex Kitchen E2E', () => {
  ...
  it('Flex Kitchen menghitung real-time, mempertahankan unknown, memberi substitusi before/after, dan menjaga ownership personal recipe', async () => {
    ...
    const consumedPreview = await app.inject({ method: 'POST', url: '/api/v1/flex-kitchen/consume', headers: owner.headers, payload: { localDate: owner.localDate, recipeName: 'Tempe dan bumbu', mealType: 'LUNCH', servings: 2, fraction: 1, ingredients: [{ foodItemId: tempe.id, servingId: tempe.servings[0].id, quantity: 2 }, { customName: 'Bumbu keluarga', quantity: 1 }] } }); expect(consumedPreview.statusCode, consumedPreview.body).toBe(200); expect(consumedPreview.json().data.nutrition.itemCount).toBe(2); expect(consumedPreview.json().data.nutrition.totals.SODIUM_MG).toBeNull();
