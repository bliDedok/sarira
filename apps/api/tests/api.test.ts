...
describe('SARIRA API v1 Phase 3', () => {
  ...
  it('menyediakan health/version Phase 3 dan melindungi route private', async () => {
    expect((await app.inject({ method: 'GET', url: '/api/v1/health' })).statusCode).toBe(200);
    expect((await app.inject({ method: 'GET', url: '/api/v1/version' })).json().data).toMatchObject({ apiVersion: 'v1', phase: 3 });
