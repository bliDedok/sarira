...
export const createBaselineRoutes = (repositories: DataRepositories, clock: Clock): FastifyPluginAsync => async (app) => {
  ...
  app.get('/baseline/current', authenticated, async (request) => {
    ...
    if (context.calendarReachedNow) await audit(request, 'BASELINE_DAY_14_REACHED', 'BaselineSession', context.baseline.id, { completenessScore: context.baseline.completenessScore });
  ...
  app.get('/baseline/:id/readiness', authenticated, async (request) => { const context = await baselineByIdContext(repositories, request.authUser!.id, baselineParams.parse(request.params).id, clock); if (!day14Available(context.currentDay)) throw new ConflictError('Readiness tersedia setelah hari ke-14.'); if (context.calendarReachedNow) await audit(request, 'BASELINE_DAY_14_REACHED', 'BaselineSession', context.baseline.id, { completenessScore: context.baseline.completenessScore }); const value = await repositories.baseline.getReadiness(context.profile.id, context.baseline.id, context.currentDay, context.now); await audit(request, value.status === 'INSUFFICIENT_DATA' ? 'BASELINE_DATA_INSUFFICIENT' : 'BASELINE_READY', 'BaselineReadinessResult', value.id, { status: value.status, completenessScore: value.completenessScore }); return success(value); });
