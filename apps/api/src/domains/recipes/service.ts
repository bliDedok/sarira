...
export async function consumeFlexKitchen(repositories: DataRepositories, clock: Clock, userId: string, input: { localDate: string; recipeName: string; mealType: 'BREAKFAST' | 'LUNCH' | 'DINNER' | 'SNACK'; servings: number; fraction: 1 | 0.75 | 0.5 | 0.25; ingredients: FlexKitchenIngredientInput[] }) {
  await assertNutritionConsent(repositories, userId); const context = await currentBaselineContext(repositories, userId, clock); const dayIndex = assertEditableDate(context, input.localDate); const allergenText = await repositories.nutrition.getProfileAllergenText(context.profile.id); const scale = input.fraction / input.servings;
  const prepared = await Promise.all(input.ingredients.map(async (ingredient) => {
    const value = await ingredientCalculation(repositories, ingredient, scale); const quantity = ingredient.quantity * scale; const missingNutrients = value.missingNutrients; const sourceVersion = value.sourceVersion;
    return { ...(value.food ? { foodItemId: value.food.id, servingId: value.serving?.id } : {}), itemSource: value.food ? 'DATABASE_FOOD' as const : ingredient.userNutrition ? 'USER_ENTERED' as const : 'CUSTOM_FOOD' as const, ...(!value.food ? { customName: value.foodName } : {}), quantity, ...(value.gramAmount ? { gramAmount: value.gramAmount } : {}), sourceVersion, snapshot: { sourceVersion, foodName: value.foodName, ...(value.gramAmount ? { gramAmount: value.gramAmount } : {}), nutrients: value.nutrition, missingNutrients, complete: missingNutrients.length === 0, calculationVersion: NUTRITION_ENGINE_VERSION }, allergenWarnings: value.food ? allergenWarnings(value.food, allergenText) : [{ code: 'OTHER' as const, label: 'Informasi alergen belum lengkap', matchedProfile: false, verified: false }] };
  }));
  const consumedAt = clock.now().toISOString(); const meal = await repositories.baseline.createMeal(context.profile.id, context.baseline.id, dayIndex, { localDate: input.localDate, mealType: input.mealType, eatenAt: consumedAt, description: input.recipeName, skipped: false, homeCooked: true }, consumedAt);
  for (const item of prepared) await repositories.nutrition.createItem({ mealLogId: meal.id, profileId: context.profile.id, localDate: input.localDate, ...item });
  return { mealLogId: meal.id, nutrition: await dailyNutrition(repositories, clock, userId, input.localDate) };
}
