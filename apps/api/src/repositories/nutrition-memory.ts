...
export function createMemoryNutritionRepository(allergenLookup: (profileId: string) => string = () => ''): NutritionRepository {
  ...
  const policies: NutritionPolicyRecord[] = DEVELOPMENT_POLICIES.map((policy) => ({ id: crypto.randomUUID(), code: policy.code, version: policy.version, ageMin: policy.ageMin, ageMax: policy.ageMax, applicableGoals: [], applicableSafetyStatuses: ['GREEN', 'YELLOW', 'RED'], targetConfiguration: { targets: policy.targets, goalEnergyAdjustmentKcal: policy.goalEnergyAdjustmentKcal }, requiresExpertValidation: true }));
