...
const stepMeta: Record<CanonicalStep, { eyebrow: string; title: string; description: string }> = {
  ...
  'program-preference': { eyebrow: 'Preferensi program', title: 'Pilih cara yang terasa realistis', description: 'Perhitungan dan rekomendasi nutrisi masih berstatus Demo.' },
...
function SummaryStep() {
  ...
  useEffect(() => { void load(); }, []);
  const complete = async () => { setSubmitting(true); setError(undefined); try { await api.completeOnboarding(); await refreshSession(); router.replace('/home?day=1' as never); } catch (cause) { setError(errorMessage(cause)); } finally { setSubmitting(false); } };
