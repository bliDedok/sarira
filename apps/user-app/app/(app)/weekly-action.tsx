...
import { AppText, Button, Card, Chip, InlineNotice, Loading, ProgressBar } from '@sarira/ui';
...
export default function WeeklyActionScreen() {
  ...
  return <AppShell title="Weekly Action" subtitle="Satu langkah kecil untuk minggu ini">
    {loading ? <Loading label="Memuat Weekly Action…" /> : !action ? <Card tone="cream" style={styles.empty}><CircleHelp size={40} color={colors.primary} /><AppText variant="h1">Weekly Action belum tersedia</AppText><AppText variant="body">{error ?? 'Buat Pattern Map setelah readiness hari ke-14 untuk memilih satu action yang eligible.'}</AppText><Button label="Buka Pattern Map" onPress={() => router.push('/pattern-map' as never)} /></Card> : <>
